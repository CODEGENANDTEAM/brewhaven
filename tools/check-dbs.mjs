// Checks every database in .env from your terminal: connects, creates the tables,
// copies the products in and counts them. Unlike the website, it prints the REAL
// error (on your own screen only), plus a hint for the usual mistakes.
//
//   npm run check
...
// Common errors, turned into a next step. The first matching hint wins.
const HINTS = [
  ...
  [/self[- ]signed|unable to get local issuer|certificate/i, "Certificate problem. Download the CA certificate and put it in the *_CA_CERT variable (see README)."],
...
for (const entry of entries) {
  ...
  try {
    const db = await entry.getDb(); // connects, creates tables, copies the products in
    await db.ping();
...
// Database pools keep the process alive, so exit explicitly.
process.exit(0);
