// Copies FIREBASE_CLIENT_EMAIL and FIREBASE_PRIVATE_KEY from a Firebase service
// account file (the .json you download from Firebase console > Project settings >
// Service accounts > Generate new private key) into .env, in the exact format the
// server expects. It never prints the key.
//
//   npm run setup:firebase                  finds the newest key file in Downloads
//   npm run setup:firebase -- path\to\key.json
//   npm run setup:firebase -- --delete      also deletes the .json afterwards
...
// Reads .env into a Map so we can check FIREBASE_PROJECT_ID.
...
// A service account file is JSON with type "service_account".
...
// Without a path, pick the newest service account file in Downloads for this project.
...
// JSON.stringify gives one quoted line with \n for the line breaks, which is
// exactly the format .env needs (and the server turns \n back into line breaks).
const lines = {
...
for (const [name, value] of Object.entries(lines)) {
  ...
  // A function replacement, so "$" characters in the value are kept as they are.
