// End to end tests for the API, using fake databases instead of real ones.
// We call the REAL entry points: the Vercel files in api/ and the Netlify function.
...
before(() => {
  // A broken fake database logs its (fake) error on purpose. Keep the output tidy.
...
// Builds a fresh API with three fake databases and plugs it into the entry points.
// Returns the fakes, so a test can break a database or look inside it.
...
// One French press, saved only to the database named in ?to=.
function placeOrderTo(to) {
  const items = [{ productId: "french-press", quantity: 1 }];
...
test("products come from the primary database, or the one in ?from=", async () => {
  ...
  assert.deepEqual(Object.keys(body.products[0]).sort(), ["category", "description", "emoji", "id", "name", "price"]);
...
test("products fall back to the next working database in a fixed order", async () => {
  ...
  fakes.mysql.control.down = true;
  // Primary (mysql) is down, so try firebase, then cockroachdb.
...
test("?to= saves a login and an order to only the chosen database", async () => {
  ...
  assert.equal(fakes.firebase.control.users.size, 0); // the others were not touched
...
test("an unknown ?to= is a 400", async () => {
  ...
  assert.equal(res2.status, 400);
  assert.equal(fakes.firebase.control.orders.length, 0); // nothing was written
});
...
test("an order uses catalog prices, merges duplicate lines and is saved everywhere with one id", async () => {
  ...
  const res = await placeOrder([
    { productId: "ethiopia-yirgacheffe", quantity: 1, price: 1 }, // the price here is ignored
    { productId: "canvas-tote", quantity: 2, unitPrice: 0 },
    { productId: "ethiopia-yirgacheffe", quantity: 2 },
  ]);
  assert.equal(res.status, 201);
  const { order, results } = await res.json();

  assert.equal(order.total, 64900 * 3 + 29900 * 2);
  assert.equal(order.currency, "INR");
  assert.equal(order.status, "placed");
  assert.deepEqual(order.items, [
    { productId: "ethiopia-yirgacheffe", name: "Ethiopia Yirgacheffe", unitPrice: 64900, quantity: 3 },
    { productId: "canvas-tote", name: "Canvas Tote", unitPrice: 29900, quantity: 2 },
  ]);
...
test("bad orders are rejected with a clear message", async () => {
  ...
  const cases = [
    ...
    [[], /items/],
    [{ items: [] }, /empty/],
    [{ items: Array.from({ length: 21 }, () => ({ productId: "canvas-tote", quantity: 1 })) }, /at most 20/],
    [{ items: [{ productId: "unicorn", quantity: 1 }] }, /not on the menu/],
    [{ items: [{ productId: "canvas-tote", quantity: 0 }] }, /1 to 10/],
    [{ items: [{ productId: "canvas-tote", quantity: 11 }] }, /1 to 10/],
    [{ items: [{ productId: "canvas-tote", quantity: 1.5 }] }, /1 to 10/],
    [{ items: [{ productId: "canvas-tote", quantity: 6 }, { productId: "canvas-tote", quantity: 5 }] }, /at most 10/],
    [{ items: ["canvas-tote"] }, /productId, quantity/],
  ];
...
test("an order is a 503 when every database fails", async () => {
  ...
  const res = await placeOrder([{ productId: "french-press", quantity: 1 }]);
  assert.equal(res.status, 503);
...
test("orders are listed per user, newest first, from ?from= with fallback", async () => {
  const fakes = setup();
  await placeOrder([{ productId: "french-press", quantity: 1 }]);
  await placeOrder([{ productId: "house-decaf", quantity: 2 }]);
  await placeOrder([{ productId: "canvas-tote", quantity: 1 }], "token-bob");
  ...
  assert.deepEqual(mine.orders.map((o) => o.items[0].productId), ["house-decaf", "french-press"]);
...
test("init runs once per database and is retried after a failure", async () => {
  ...
  await health.GET(request("/api/health"));
  assert.equal(fakes.firebase.control.initCalls, 1); // cached after the first success
  assert.equal(fakes.mysql.control.initCalls, 2); // failed, so tried again
  ...
  assert.equal(fakes.mysql.control.products.length, 8); // the catalog was copied in
});
...
test("a missing FIREBASE_PROJECT_ID is a 500 that names the variable", async () => {
  setApiForTesting(null); // use the real build() this time
